...
if(!identical(roundup_remove_list$root_input$dir_date_version, roundup_remove_list$root_output$dir_date_version)){
...
Note: The tool tries to resolve all operations in each `root` independently.  So even though we're not creating a folder in both our `root`s, the tool will create as many logs as it can.

_**If you need to handle roots independently, then you should instantiate different versions of the tool to handle each independent root, giving each instance of the tool a unique name e.t. `slt_input` and `slt_output`**._

```{r make_new_log}
# Make a naive folder without a log
dir.create(file.path(root_output, "2024_04_10_naive"))
try(slt$make_new_log(date_version = "2024_04_10_naive"))
```
...
```{r print}
# Print all the static internal fields.
...
```{r print2}
# Call without args to show everything
slt$print_dictionaries()
```

```{r print3}
# Show the last 'action' the tool performed
# - these fields reset each time the user defines a new action, and are set as part of the 'marking' operations.
