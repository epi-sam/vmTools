...
```{r setup}
# devtools::load_all() # for interactive use
library(vmTools) # to build vignettes
library(data.table)
```
...
```{r reports}
fread(file.path(root_input, "report_key_versions.csv"))
```

```{r reports2}
fread(file.path(root_output, "report_key_versions.csv"))
```
...
slt$create_date_version_folders_with_logs(version_name = date_version_today)
slt$create_date_version_folders_with_logs(version_name = date_version_today)
```
...
suppressMessages({
  slt$mark_best(version_name = date_version_today, user_entry = list(comment = "testing mark_best"))
})
fread(PATHS$log_cent)
```
...
```{r demote_best}
suppressMessages({
  slt$unmark(version_name = date_version_today, user_entry = list(comment = "testing unmark_best"))
})
print_tree()
```
...
```{r mark_remove}
suppressMessages({
  slt$mark_remove(version_name = date_version_today, user_entry = list(comment = "testing mark_remove"))
})
```
...
```{r delete_folder2}
slt$delete_date_version_folders(
  version_name       = date_version_today,
  user_entry         = list(comment = "testing delete_date_version_folders"),
